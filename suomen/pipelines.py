...
class SuomenPipeline:

    # Database setup
    conn = sqlite3.connect('suomen.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `suomen`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM suomen WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO suomen (date, title, link, content)"
